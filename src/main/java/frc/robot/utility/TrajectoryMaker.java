...
public TrajectoryMaker(Pose2d start, Pose2d end, ArrayList<Translation2d> points) {
    ...
    config = new TrajectoryConfig(10, 3.5);  // (TrajectoryConfig​(double maxVelocityMetersPerSecond, double maxAccelerationMetersPerSecondSq))
    config.setStartVelocity(0);
    config.setEndVelocity(0);
    config.setReversed(false);
    config.addConstraint(new CentripetalAccelerationConstraint(3.5));
    listOfPoints = points;
    trajectory = createTrajectory();
}
