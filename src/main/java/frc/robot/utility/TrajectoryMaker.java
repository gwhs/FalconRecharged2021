...
public TrajectoryMaker(Pose2d start, Pose2d end, ArrayList<Translation2d> points) {
    ...
    config = new TrajectoryConfig(5, 2.5);  // (TrajectoryConfig​(double maxVelocityMetersPerSecond, double maxAccelerationMetersPerSecondSq))
    config.setStartVelocity(0);
    config.setEndVelocity(0);
    config.setReversed(false);
    config.addConstraint(new CentripetalAccelerationConstraint(2.25));
    listOfPoints = points;
    trajectory = createTrajectory();
}
